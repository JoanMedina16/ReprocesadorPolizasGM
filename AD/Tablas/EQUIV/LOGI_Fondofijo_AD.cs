...
{
   public class LOGI_Fondofijo_AD
    {
        internal Hashtable oHashParam = null;
        void GetObjeto(DataRow objRow, ref LOGI_Catalogos_INFO oBanco)
        {
            oBanco.sAX365 = objRow["IdFodofijo"] == DBNull.Value ? "" : Convert.ToString(objRow["IdFodofijo"]).Trim();
        ...
        public string RecuperaFondofijo(ref LOGI_ConexionSql_AD oConnection, ref List<LOGI_Catalogos_INFO> lstBancos, LOGI_Catalogos_INFO oBanco, out string sConsultaSql, bool bTop)
        {
            DataSet odataset = new DataSet();
            sConsultaSql = string.Empty;
            bool bAnd = false;
            string sTop = bTop ? " TOP 150 " : "";
            sConsultaSql = string.Format(@"SELECT {0} T1.*, T2.IdEmpresa, T2.sNombrerazon FROM cat_fondofijo T1 INNER JOIN cnf_empresa T2 ON 
