...
namespace PD.Tablas.D365
{
    public class LOGI_Polizas_PD
    {
        ...
        public string InsertaPeticion(string sUsuarioID, LOGI_Peticion_INFO oParam, ref List<LOGI_Peticion_INFO> lstPeticiones, int Top = 0, bool bAscendete = false)
        {
            string sReponse = string.Empty, sConsultaSql = string.Empty;
            sReponse = "ERROR";
            try
            {
                ...
                oConnection.OpenConnection();
                sReponse = oPolizas.InsertaPeticion(ref oConnection, ref lstPeticiones, oParam, out sConsultaSql, Top, bAscendete);
            }
            catch (Exception ex)
            {
                oTool.LogError(ex, "InsertaPeticion", sUsuarioID, CONST_CLASE, CONST_MODULO, sDatosAdicionales: sConsultaSql);
                sReponse = string.Format("No se ha podido insertar la petición {0}", ex.Message);
            }
            finally
            {
                if (oConnection != null)
